        ...
        <div className="p-6 space-y-4 overflow-y-auto">
          ...
          <div>
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              {/* Dropdown */}
              {showActorDropdown && (newActorName || actors.length > 0) && (
                <div className="absolute z-10 w-full mt-1 bg-gray-700 border border-gray-600 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                  {filteredActors.length > 0 ? (
                    <>
                      <div className="px-3 py-2 text-xs text-gray-400 border-b border-gray-600">
                        Recent Actors (Last 30 min)
                      </div>
                      {filteredActors.map((actor, index) => (
                        <button
                          key={index}
                          onClick={() => addActor(actor)}
                          className="w-full px-4 py-2.5 text-left hover:bg-gray-600 transition-colors flex items-center justify-between"
                        >
